  ...
  */
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mobile Filter Tabs (Pills) */}
      <div className="md:hidden flex gap-2 overflow-x-auto pb-2 no-scrollbar">
        {['Todos', 'Despesas', 'Receitas', 'Concluídos'].map(tabLabel => {
          const tabid = tabLabel === 'Todos' ? '' : (tabLabel === 'Despesas' ? 'despesa' : (tabLabel === 'Receitas' ? 'receita' : 'concluidos'))
          const isActive = tabid === '' ? (activeTab !== 'concluidos' && activeTab !== 'receita' && activeTab !== 'despesa') : tabid === activeTab
          // Mapping 'Todos' is tricky because logic uses activeTab state.
          // Let's rely on standard mapping. If 'Todos' is selected, maybe we show everything?
          // Current logic separates by activeTab.
          // Let's simplify: Despesas, Receitas, Concluídos. What about 'Todos'?
          // The screenshot shows 'Todos', 'Despesas', 'Receitas', 'Concluídos'.
          // We can map 'Todos' to a state where we show all? Or maybe just keep existing tabs.
          // Let's align with existing 'activeTab'.
          // Existing: despesa, receita, retirada, aporte, concluidos.
          // Mobile 'Todos' might mean "All Active"?
          // For now let's use the explicit tabs to match logic. 
          // Or better, adapt the pill to set activeTab.

          const map: any = { 'Despesas': 'despesa', 'Receitas': 'receita', 'Concluídos': 'concluidos' }
          const id = map[tabLabel]

          if (tabLabel === 'Todos') {
            // We don't have a 'Todos' mode easily without refactoring 'byTab' logic.
            // Skip 'Todos' for now or make it 'despesa' default?
            // Let's stick to: Despesas, Receitas, Concluídos.
            return null
          }
