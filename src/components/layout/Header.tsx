      ...
      {showTxModal && (
        <TransactionModal
          ...
          onSuccess={() => {
            setShowTxModal(false)
            setSelectedFav(null)
            // Optionally reload data if on Ledger page, but global reload is hard.
            // Assuming realtime or user manual refresh for now.
            window.location.reload() // Force reload to see changes everywhere for now
          }}
          initialData={selectedFav ? {
            // Map favorite fields to transaction fields if needed, 
            // but assuming structure is similar enough or TransactionModal handles it.
            // TransactionModal expects 'initialData' to pre-fill form.
            // We need to match the keys TransactionModal expects.
